...
class DeleteByName : ComponentActivity() {
...
@Composable
fun DeleteByName(repository: CapitalRepository) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Eliminar Ciudad Capital") },
                navigationIcon = {
                    IconButton(onClick = {
        ...
        Column(
            ...
            Subtitle(string = "Ingrese el nombre de la capital a eliminar", modifier = Modifier.padding(16.dp))
            OutlinedTextField(
                value = capitalName,
            ...
            capitalResult?.let { capital ->
                Card(modifier = Modifier
                    ...
                    .padding(4.dp)
                    .clickable {
                        val success = repository.deleteCapital(capital.id)
                        if (success) {
                            capitalResult = null
                            Toast.makeText(context, "Capital eliminada", Toast.LENGTH_SHORT).show()
                        } else {
                            Toast.makeText(context, "Error al eliminar la capital", Toast.LENGTH_SHORT).show()
                        }
                    }
                ) {
                    Column(modifier = Modifier.padding(16.dp)) {
                        ...
                        Spacer(modifier = Modifier.height(16.dp))
                        Text(text = "Haz clic para eliminar", fontSize = 14.sp, color = MaterialTheme.colorScheme.error)
                    }
                }
            }
        }
